...
def close_connections(selector, sock):
...
def read_callback(selector: selectors.BaseSelector, sock: socket.socket):
    ...
    if data:
        ...
        try:
            data_decoded = data.decode(errors='ignore').rstrip('\r\n')
            if data_decoded == 'quit':
                close_connections(selector, sock)
            elif data_decoded == 'time':
                time = datetime.now().strftime('%H:%M - %d.%m.%Y года')
                sock.send(time.encode())
            else:
                sock.send(data)
        except UnicodeDecodeError:
            sock.send('unable to decode message'.encode())
    else:
        close_connections(selector, sock)
